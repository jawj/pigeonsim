...
class this.FeatureManager
  constructor: (@ge) ->
    @featureTree = new RTree(0.05)  # max 0.05 degrees of lat/lon
    @visibleFeatures = {}
    @maxVisible = 20
    @featureFifo = []
  ...
  showFeature: (f) ->
    ...
    @ge.getFeatures().appendChild(f.geNode)
  ...
  hideFeature: (f) ->
    ...
    @ge.getFeatures().removeChild(f.geNode)
    delete f.geNode
    return yes
  ...
  updateWithBBox: (lat1, lon1, lat2, lon2) ->
    @showFeature(f) for f in @featuresInBBox(lat1, lon1, lat2, lon2)
...
class this.FeatureSet
  ...
  loadFeatures: ->  # override in subclasses
