(function() {
  ...
  this.FeatureManager = (function() {
    ...
    function FeatureManager(ge) {
      this.ge = ge;
      this.featureTree = new RTree(0.05);
    ...
    FeatureManager.prototype.showFeature = function(f) {
      ...
      st = new SkyText(f.lat, f.lon, f.alt);
      f.render(st);
      f.geNode = ge.parseKml(st.kml());
      this.ge.getFeatures().appendChild(f.geNode);
    ...
    FeatureManager.prototype.hideFeature = function(f) {
      ...
      this.ge.getFeatures().removeChild(f.geNode);
      delete f.geNode;
      return true;
    };
    ...
    FeatureManager.prototype.updateWithBBox = function(lat1, lon1, lat2, lon2) {
      var f, _i, _len, _ref, _results;
      _ref = this.featuresInBBox(lat1, lon1, lat2, lon2);
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        f = _ref[_i];
        _results.push(this.showFeature(f));
      }
      return _results;
    };
  ...
  this.Feature = (function() {
    ...
    Feature.prototype.render = function(st) {
      return st.line('[Abstract feature]');
    };
  ...
  this.TubeStation = (function(_super) {
    ...
    TubeStation.prototype.render = function(st) {
      return st.line("ø " + this.name);
    };
  ...
  this.TubeStationSet = (function(_super) {
    ...
    function TubeStationSet() {
      TubeStationSet.__super__.constructor.apply(this, arguments);
    }

    TubeStationSet.prototype.loadFeatures = function() {
      var code, dummy, lat, lon, name, row, station, _i, _len, _ref, _ref2, _results;
      _ref = this.csv.split("\n");
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        row = _ref[_i];
        _ref2 = row.split(','), code = _ref2[0], dummy = _ref2[1], lon = _ref2[2], lat = _ref2[3], name = _ref2[4];
        console.log(code, dummy, lon, lat, name);
        station = new TubeStation("tube-" + code, lat, lon);
        station.name = name;
        _results.push(this.addFeature(station));
      }
      return _results;
    };

    TubeStationSet.prototype.csv = "ACT,DP,-0.28025120353611,51.50274977300050,Acton Town\nALD,MH,-0.07561418447775,51.51427182308330,Aldgate\nALE,DH,-0.07228711997537,51.51523341379640,Aldgate East\nALP,P,-0.29948653867861,51.54069476629340,Alperton\nAME,M,-0.60747883910247,51.67414971062970,Amersham";
